...
# 추상화된 데이터와 함수를 하나의 단위로 묶어 클래스를 만들 수 있고, 클래스를 사용해 인스턴스를 생성하여 객체 단위로 사용 가능하다.
...
# 클래스 속성 : 클래스 자체에 속하는 변수로, 모든 인스턴스가 공유하는 속성
...
# 메소드 : 클래스 내부에 정의된 함수로, 인스턴스의 데이터를 조작하거나 동작을 정의한다.
    # self : 필드 및 메소드에 접근하는 객체를 의미
    #       객체를 통해 접근시 호출되는 메소드의 첫 번째 인자로 항상 self를 넘겨주어야한다.
...
# 생성자 : __init__ (언더바 2개 씩) : 객체가 생상될 때 자동으로 호출된다. 매개변수를 전달 받아 인스턴스 속성을 초기화 할 수 있다. 
...
# 네임 스페이스는 크게 다섯가지로 나누어볼 수 있다.
# 1. 지역 네임스페이스 : 현재 함수나 메소드 내의 네임 스페이스
# 2. 인스턴스 네임 스페이스 : 인스턴스 객체의 네임 스페이스
# 3. 클래스 네임 스페이스 : 클래스 객체의 네임 스페이스
# 4. 전역 네임스페이스 : 모듈 내의 전역 네임 스페이스
# 5. 내장 네임 스페이스 : 파이썬 내장 함수와 예외를 포함하는 네임스페이스
...
class Test_class:
    ...
    def __init__(self,value):
        self.variable = value # 인스턴스 네임 스페이스
...
### Global ###
# 함수 내부에서 전역 변수를 참조하거나 수정할 때이용하며, 함수 내부에서 전역 변수에 접근가능
# 설정 값을 전역적으로 유지하고 여러ㅓ 함수에서 이 값을 변경하거나 참조할 때 유용하다.
...
### nonlocal ###
# 중첩함수에서 바깥 함수의 변수를 참조하거나 수정할 때 사용
...
# 파이썬은 private가 쓰이지 않는다.
# 단지 접근하기 불편하게 만들어서 개발자들간의 약속을 만듬.

class BankAccount : 
    
    def __init__ (self, account_number, balance):
        self.account_number = account_number # 공개속성
        self.__balance = balance             # __ 비공개 속성(약속)
    ...
    def deposit(self, amount):
        if amount > 0 :
            ...
            raise ValueError('입금 금액은 양수여야합니다.')
...
# 비공개 속성에 직접 접근 시도
# 언더 스코어 2개로 시작하는 속성은 완전한 의미의 private 은 아니지만, 어느정도 보호되느 ㄴ속성으로 취급.
...
print("입금 . 후잔액 : ", my_account.get_balance())
